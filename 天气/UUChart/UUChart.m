...
-(void)setUpChart{
	if (self.chartStyle == UUChartLineStyle) {
        ...
        //显示竖线
        if ([self.dataSource respondsToSelector:@selector(UUChart:ShowVerticalLineAtIndex:)]) {
            NSMutableArray *showVerticalArray = [[NSMutableArray alloc]init];
            for (int i=0; i<5; i++) {
                if ([self.dataSource UUChart:self ShowVerticalLineAtIndex:i]) {
                    [showVerticalArray addObject:@"1"];
                }else{
                    [showVerticalArray addObject:@"0"];
                }
            }
            [_lineChart setShowVerticalLine:showVerticalArray];
