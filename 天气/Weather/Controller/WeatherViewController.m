...
@interface WeatherViewController ()<CLLocationManagerDelegate,AVSpeechSynthesizerDelegate>
//背景图
...
- (void)setupSubView{
    ...
    cityLabel.text = [NSString stringWithFormat:@"%@ %@",_city,_weizhi];
...
- (void)speech:(UIButton *)button {
    ...
    _speechText = [NSString stringWithFormat:@"%@当前天气状况%@。最高气温%@摄氏度。最低气温%@摄氏度。现在气温%@。空气质量为%@",_city,_weatherCond,_maxDegrees,_minDegrees,_degrees,_kongQi];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray<CLLocation *> *)locations{
    ...
    [coder reverseGeocodeLocation:location completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
        ...
        NSString * dingwei = [NSString stringWithFormat:@"%@",mark.subLocality];
        self.weizhi = dingwei;
        ...
        [manager GET:NOWWEATHER parameters:dic progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            //城市
            NSString * city = [NSString stringWithFormat:@"%@市",[responseObject[@"HeWeather5"] firstObject][@"basic"][@"city"]];
            //更新时间
           ...
           dispatch_async(dispatch_get_main_queue(), ^{
               ...
               tempSelf.vis = vis;
               tempSelf.wind = wind;
               NSString * shikuangStr = [NSString stringWithFormat:@"%@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@",_city,_updateTime,_weatherCond,_degrees,_hum,_pcpn,_vis,_wind,_condstr,_sunrise,_sunset,_weizhi,_maxDegrees,_minDegrees,_kongQi];
               [_manager deleteTable:@"shikuang"];
               [_manager createTableViewName:@"shikuang"];
               [_manager insertDataToTableName:@"shikuang" WithCTName:shikuangStr];
               //[tempSelf refresh];
           });
        ...
        [manager GET:FORECASTWEATHER parameters:dic progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSString * threeStr = [NSString stringWithFormat:@"%@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@",_firstDate,_twoDate,_threeDate,_twoMaxDegrees,_twoMinDegrees,_threeMaxDegrees,_threeMinDegrees,_twoSunrise,_twoSunset,_threeSunrise,_threeSunset,_twoWeatherCond,_threeWeatherCond];
                [_manager deleteTable:@"three"];
                [_manager createTableViewName:@"three"];
                [_manager insertDataToTableName:@"three" WithCTName:threeStr];
                //[tempSelf refresh];
            });
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        ...
        [manager GET:LIFE parameters:dic progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSString * zhishuStr = [NSString stringWithFormat:@"%@ %@ %@ %@ %@ %@ %@",_xiChe,_chuanYi,_shuShi,_ganMao,_lvYou,_yunDong,_ziWaiXian];
                [_manager deleteTable:@"zhishu"];
                [_manager createTableViewName:@"zhishu"];
                [_manager insertDataToTableName:@"zhishu" WithCTName:zhishuStr];
                [tempSelf refresh];
        ...
        {
            UIAlertController * alert = [UIAlertController alertControllerWithTitle:nil message:@"定位未完成请刷新" preferredStyle:UIAlertControllerStyleAlert];
            [self showDetailViewController:alert sender:nil];
        ...
        {
            UIAlertController * alert = [UIAlertController alertControllerWithTitle:nil message:@"网络连接有点问题请检查" preferredStyle:UIAlertControllerStyleAlert];
            [self showDetailViewController:alert sender:nil];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self dismissViewControllerAnimated:YES completion:nil];
            });
